...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser: Any) -> None:
        ...
        parser.add_argument(
            ...
            help="Maximum number of snippets to send in one embedding request.",
        )
        parser.add_argument(
            "--dry-run",
            action="store_true",
        ...
        parser.add_argument(
            "--skip-errors",
            action="store_true",
            help="Continue processing later snippets when one embedding request fails.",
        )
    ...
    def handle(self, *args: Any, **options: Any) -> None:
        ...
        updated = 0
        failed = 0
        for index in range(0, len(snippets), batch_size):
            batch = snippets[index : index + batch_size]
            texts = [build_knowledge_snippet_embedding_text(snippet) for snippet in batch]
            try:
                ...
                if not skip_errors:
                    raise
                results = []
                for snippet, text in zip(batch, texts, strict=True):
                    try:
                        ...
                        self.stderr.write(
                            self.style.WARNING(
                                f"failed id={snippet.id} title={snippet.title!r}: "
                                f"{type(fallback_exc).__name__}: {fallback_exc}",
                            ),
                        )
                        results.append(None)

            for snippet, result in zip(batch, results, strict=True):
                if result is None:
                    continue
